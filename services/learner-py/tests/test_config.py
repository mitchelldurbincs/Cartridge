...
def test_load_config(tmp_path: Path) -> None:
    ...
    config_payload = {
        ...
        "weights": {"backend": "grpc", "endpoint": "localhost:50052"},
        "control": {
...
def test_redis_backend_requires_channel(tmp_path: Path) -> None:
    config_path = tmp_path / "config.json"
    config_payload = {
        "replay": {"endpoint": "localhost:50051", "batch_size": 32},
        "training": {
        ...
        "algorithm": {"name": "ppo"},
        "checkpoints": {"bucket": str(tmp_path / "ckpts"), "interval_steps": 10, "keep_last": 2},
        "weights": {"backend": "redis", "endpoint": "redis://localhost:6379"},
        "control": {
            "orchestrator_endpoint": "http://localhost:8000",
            "run_id": "test-run",
            "heartbeat_interval_seconds": 30,
