...
| `learner_sample_results_total` | Counter | `result` (`success`, `error`, `ok`, `empty`) | Breaks down replay sampling outcomes: successes and transport errors from the prefetch loop, plus end-to-end batch fetches in the training loop. The `empty` result is recorded when the replay buffer has no transitions available. | Replay client success/error paths and learner core `_fetch_batch`.
...
| `engine_registry_create_failures_total` | Counter | `env_id` | Counts failed attempts to create game instances for unregistered environment IDs. | `create_game` in `engine-core/src/registry.rs` when environment is not found. |
...
| `engine_batch_simulate_trajectories_total` | Counter | _none_ | Total number of trajectories requested across all BatchSimulate RPC calls. | Incremented at the start of `batch_simulate` handler in `engine-server/src/service.rs`. |
| `engine_batch_simulate_trajectory_failures_total` | Counter | `error` (`create_failed`, `reset_failed`, `action_sample_failed`, `step_failed`) | Counts trajectory simulation failures by error type during BatchSimulate streams. | Error handling paths within the `batch_simulate` stream in `engine-server/src/service.rs`. |
| `engine_batch_simulate_steps_total` | Counter | _none_ | Total simulation steps executed across all trajectories in BatchSimulate calls. | After completing each trajectory in `batch_simulate` stream in `engine-server/src/service.rs`. |
| `engine_batch_simulate_trajectory_steps` | Histogram | _none_ | Distribution of steps per trajectory in BatchSimulate calls. | After completing each trajectory in `batch_simulate` stream in `engine-server/src/service.rs`. |
| `engine_batch_simulate_trajectory_reward` | Histogram | _none_ | Distribution of cumulative rewards per trajectory in BatchSimulate calls. | After completing each trajectory in `batch_simulate` stream in `engine-server/src/service.rs`. |
